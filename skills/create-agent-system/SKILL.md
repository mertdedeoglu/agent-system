...
5. Check whether a `clean-code-standards` skill/reference is present in the user's profile or project (e.g. referenced in other `.claude/` config, or the user mentions it). If so, note this — the implementer and reviewer templates below have a placeholder to link to it explicitly.
...
- If `clean-code-standards` was detected in Step 1, uncomment/enable the "Defer to clean-code-standards" line in each implementer/reviewer template. Otherwise leave the inline SOLID/DRY/KISS/YAGNI baseline already in the templates as the fallback.
...
- Whether `clean-code-standards` was linked in
